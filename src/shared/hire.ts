 ...
 * `sovereignhive://hire?src=<https-url>` deep link or an in-app file picker.
 *
...
export const HIRE_SPEC_V1 = 'sovereign-hive/hire@1';
...
export interface HireManifest {
  /** Spec tag; `sovereign-hive/hire@1` (or legacy `munder-difflin/hire@1`). */
...
/** Parse a `sovereignhive://hire?src=<https-url>` deep link. Returns the https
...
export function parseHireDeepLink(link: string): string | null {
  ...
  if (u.protocol !== 'sovereignhive:' && u.protocol !== 'munderdifflin:') return null;
  // Both sovereignhive://hire?src= (host) and sovereignhive:hire?src= (path).
